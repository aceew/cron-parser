...
/**
 * Take a single expression and the possible values and returns an array of the possible times it
 * could run.
 * @param expression Cron expression, this does not support lists. For lists use
 * calculateValuesForExpression
...
const calculateExpression = (
  ...
  Object.keys(requirements.alternativeValueMap).forEach((alternativeKey) => {
    ...
    expression = expression
      .replace(searchKey, String(requirements.alternativeValueMap[alternativeKey]));
  });

  // Step values are for numbers that divide equally, e.g running every 5th minute
  if (expression.includes(STEP_VALUE)) {
    const [, divisible] = expression.split(STEP_VALUE);
 ...
 */
const calculateValuesForExpression = (
  expression: string,
  requirements: UnitRequirements,
): number[] => {
